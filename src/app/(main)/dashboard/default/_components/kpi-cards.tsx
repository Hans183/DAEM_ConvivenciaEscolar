...
export function KpiCards({ data, isAdmin }: KpiCardsProps) {
  const pctMedidas = data.totalDec > 0 ? Math.round((data.medidasEfectivas / data.totalDec) * 100) : 0;
  ...
  return (
    <div className={cn("grid gap-4", isAdmin ? "grid-cols-2 lg:grid-cols-4" : "grid-cols-1 sm:grid-cols-3")}>
      ...
      <KpiCard
        ...
        color="orange"
        trend={
          data.protocolosTrend !== undefined ? { value: data.protocolosTrend, label: "vs mes anterior" } : undefined
        }
      />
      <KpiCard
